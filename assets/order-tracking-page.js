$(document).ready(function () {
  $(".tracker-button").click(function () {
    ...
    if (!regexPattern.test(inputValue)) {
      ...
      $(".number-step").hide();
      var url =
        "https://bitools.bombayshavingcompany.com/index.php/authentication/tracking/" +
        inputValue;
      $.ajax({
        url: url,
        type: "GET",
        success: function (data) {
          ...
          console.log(
            "Estimated Delivery Date: " + formattedEstimatedDeliveryDate
          );
          ...
          var formattedOrderDate =
            getDayWithSuffix(orderDay) + " " + orderMonth + " " + orderYear;
          ...
          if (
            deliveryPartner === "Delhivery Direct Sur" ||
            deliveryPartner === "Delhivery Surface"
          ) {
            $("#delivery-partner-logo").html(
              '<h2><img src="https://cdn.shopify.com/s/files/1/0459/6563/9834/files/Delhivery.png?v=1697526138" alt="delivery logo" style="width:30px; height:30px"></h2>'
            );
          }
          if (deliveryDate === null) {
            ...
            $("#delivery-date").html(
              "<h3>Delivered On</h3><h2>" + formattedDate + "</h2>"
            );
          }
          $("#order-summary").html(
            "<tbody><tr><td>Order Name</td><td>" +
              orderName +
              "</td></tr><tr><td>Order Placed On</td><td>" +
              formattedOrderDate +
              "</td></tr><tr><td>Order Total</td><td>" +
              orderAmount +
          ...
          $("#tracking").html(
            '<p>Tracking ID:<br><span style="font-weight:500; color:blue">' +
              trackingId +
              "</span></p>"
          );
          ...
          $(".heading-otp").html(
            "<h2>Hi, " +
              customerName +
              "</h2><p>Based on your Order ID/AWB, here are your order details</p>"
          );
          const statuses = [
            "Open",
            "Picked",
          ...
          if (
            ...
            var shipStatusUpdate = "Order Confirmed";
            $("#est-del-status").html(
            ...
            shipStatus === "LostInTransit"
          ) {
            var shipStatusUpdate = "Shipped";
            $("#est-del-status").html(
              '<p style="margin-left:10px; color:#fff; font-size:15px">Estimated date of Delivery: <span class="del-date">' +
                formattedEstimatedDeliveryDate +
                "</span></p>"
            );
